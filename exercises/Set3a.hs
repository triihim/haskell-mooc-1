...
firstHalf s = take (divAndCeiling (length s) 2) s 
              where divAndCeiling x y = (div x y) + (mod x y)

palindrome :: String -> Bool
palindrome s = s == reverse s -- Case sensitive
...
helper :: (a -> Either b a) -> Either b a -> b
helper f (Right x) = helper f (f x)
helper f (Left x) = x
...
joinToLength n arr = [i++j | i<-arr, j<-arr, length (i++j) == n]
...
sumRights xs = sum (map (either (\l->0) (\r->r)) xs)
...
multiCompose [] x = x
multiCompose fs x = foldl (.) (head fs) (tail fs) x
...
interpreter commands = reverse $ go commands 0 0 []
                       where 
                           go [] x y result = result
                           go (c:cmds) x y result = case c of
                               "up" -> go cmds x (y+1) result
                               "down" -> go cmds x (y-1) result
                               "left" -> go cmds (x-1) y result 
                               "right" -> go cmds (x+1) y result
                               "printY" -> go cmds x y ((show y):result)
                               "printX" -> go cmds x y ((show x):result)