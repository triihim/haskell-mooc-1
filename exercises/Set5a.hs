...
totalPrice (MkShoppingEntry name uprice count) = uprice * fromIntegral count
...
buyOneMore (MkShoppingEntry name uprice count) = MkShoppingEntry name uprice (count+1)
...
study (NthYear a) = if a < 7 then NthYear (a+1) else Graduated
...
fromBin :: Bin -> Int
fromBin b = convert b 0 0
            where convert (O b) e d = convert b (e+1) d
                  convert (I b) e d = convert b (e+1) (d+2^e)
                  convert End _ d = d
...
toBin' :: Int -> Int -> Bin
toBin' q r = if q > 0
             then bit (mod q 2) (toBin' (div q 2) (mod q 2)) 
             else End
             where bit 0 n = O n
                   bit 1 n = I n
