...
oddFactorial n = oddFactorial' n 1

oddFactorial' :: Integer -> Integer -> Integer
oddFactorial' n a = if n <= 1 then a
                    else oddFactorial' (x-2) (a*x)
                    where x = if odd n then n else n-1
...
smallestDivisor' n k = if remainder == 0
                       then k
                       else smallestDivisor' n (k+1)
                       where remainder = mod n k
...
isPrime 0 = False
isPrime 1 = False
isPrime n = divider == 1 || divider == n
            where divider = smallestDivisor n
