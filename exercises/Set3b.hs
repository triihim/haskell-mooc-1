...
sums' :: Int -> [Int] -> [Int]
sums' 0 ls = ls
sums' i ls = sums' (i-1) ((upto i 0):ls)
             where upto 0 a = a
                   upto n a = upto (n-1) (a+n)
...
indexDefault xs i def = indexDefault' xs i 0 def

indexDefault' [] _ _ def = def
indexDefault' (x:xs) i n def
      | n == i = x
      | otherwise = indexDefault' xs i (n+1) def
...
sumsOf [] = []
sumsOf (a:[]) = [a]
sumsOf (a:b:rest) = a:(sumsOf ((a+b):rest))
