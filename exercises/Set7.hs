...
data State = Start | Eggs | Flour | Sugar | Mixing | Mixed | Baked | Error | Finished
  deriving (Eq,Show)
...
reverseNonEmpty (x:|xs) = let reversed = reverse (x:xs)
                          in (head reversed):|(tail reversed)
...
passwordAllowed p (ContainsSome s) = s == "" || length (filter (\c -> elem c p) s) > 0
...
operation "+" l1 l2 = (Operation "+" l1 l2)
operation "*" l1 l2 = (Operation "*" l1 l2)

evaluate :: Arithmetic -> Integer
evaluate (Literal v) = v
evaluate (Operation "+" l1 l2) = (evaluate l1) + (evaluate l2)
evaluate (Operation "*" l1 l2) = (evaluate l1) * (evaluate l2)

render :: Arithmetic -> String
render (Literal v) = show v
render (Operation "+" l1 l2) = "(" ++ (render l1) ++ "+" ++ (render l2) ++ ")"
render (Operation "*" l1 l2) = "(" ++ (render l1) ++ "*" ++ (render l2) ++ ")"
